                    ...
                    <div class="card custom-card">
                        <div class="card-header">
                            <div class="card-title">
                                <i class="ri-article-line me-2"></i>Create New Blog Post
                            </div>
                            <p class="text-muted mb-0">Fill in the details below to publish a new blog article</p>
                        </div>
                        <div class="card-body">
                            <form action="{{ route('admin.blogs.store') }}" method="POST" class="needs-validation"
                                ...
                                <div class="row">
                                  <div class="col-md-6 mb-4">
                                    ...
                                    <div class="col-md-6 mb-4">
                                        ...
                                        <div class="input-group">
                                            <input type="text" id="blog-image" name="image"
                                                class="form-control @error('image') is-invalid @enderror"
                                                value="{{ old('image') }}"
                                                placeholder="Image URL" readonly>
                                            <input type="hidden" id="blog-media-id" name="media_id" value="{{ old('media_id') }}">
                                ...
                                <div class="row">
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input type="text" name="meta_title" id="meta_title"
                                                    ...
                                                    maxlength="70" value="{{ old('meta_title') }}">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <textarea name="meta_description" id="meta_description"
                                                    class="form-control @error('meta_description') is-invalid @enderror" maxlength="160" rows="2">{{ old('meta_description') }}</textarea>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <small class="form-text text-muted">The main keyword you want to rank
                                                    for</small>
                                                @error('focus_keyword')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <small class="form-text text-muted">Leave blank to use featured
                                                    image.</small>
                                                @error('meta_image')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select name="robots" id="robots"
                                                    ...
                                                    <option value="noindex, nofollow"
                                                        {{ old('robots') == 'noindex, nofollow' ? 'selected' : '' }}>
                                                        Noindex, Nofollow</option>
                                                    <option value="noindex, follow"
                                                        {{ old('robots') == 'noindex, follow' ? 'selected' : '' }}>Noindex,
                                                        Follow</option>
                                    ...
                                    <div id="seo-analysis" class="mb-3">
                                        <strong>SEO Analysis</strong>
                                        <ul class="list-unstyled mt-2">
                                            <li>
                                                <span id="dot-title" class="seo-dot"></span>
                                                <span id="title-status" class="badge bg-secondary">Title: -</span>
                                                <small id="title-length" class="ms-2"></small>
                                            </li>
                                            <li>
                                                <span id="dot-desc" class="seo-dot"></span>
                                                <span id="desc-status" class="badge bg-secondary">Description: -</span>
                                                <small id="desc-length" class="ms-2"></small>
                                            </li>
                                            <li>
                                                <span id="dot-keyword" class="seo-dot"></span>
                                                <span id="keyword-status" class="badge bg-secondary">Focus Keyword:
                                                    -</span>
                                                <small id="keyword-check" class="ms-2"></small>
                                            </li>
                                        </ul>
                                    </div>

                                    <!-- Readability Analysis -->
                                    <div id="readability-analysis" class="mb-3">
                                        <strong>Readability</strong>
                                        <ul class="list-unstyled mt-2">
                                            <li>
                                                <span id="dot-readability" class="seo-dot"></span>
                                                <span id="readability-score" class="badge bg-secondary">Readability:
                                                    -</span>
                                                <small id="readability-detail" class="ms-2"></small>
                                            </li>
                                        </ul>
                                    </div>
                                    <div id="yoast-extra-analysis" class="mb-3">
                                        <strong>Extra SEO Checks</strong>
                                        <ul class="list-unstyled mt-2" id="extra-checks-list"></ul>
                                    </div>

                                    <!-- Social Preview Card -->
                                    <div class="card mt-4 mb-4" id="social-preview">
                                        <div class="card-header">Social Media Preview</div>
                                        <div class="card-body">
                                            <div class="d-flex">
                                                <img id="preview-image" src="/images/no-image.png"
                                                    style="width:100px;height:100px;object-fit:cover;border-radius:8px;margin-right:16px;"
                                                    alt="Social Image">
                                                <div>
                                                    <div id="preview-title" style="font-weight:bold;font-size:1.2rem;">
                                                    </div>
                                                    <div id="preview-url" style="color:#3490dc;font-size:0.9rem;"></div>
                                                    <div id="preview-description"
                                                        style="color:#6c757d;font-size:0.95rem;margin-top:8px;"></div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="d-flex justify-content-between align-items-center border-top pt-4">
                                    ...
                                    <button type="submit" class="btn btn-primary">
                                        <i class="ri-save-line me-1"></i> Publish Blog
                                    </button>
                                </div>
                            </div>
                            </form>
...
<script>
    ...
    // Stopwords list (for better matching)
    const stopwords = ['the', 'a', 'an', 'your', 'my', 'our', 'and', 'or', 'to', 'in', 'of', 'by', 'with', 'for', 'on', 'at', 'from', 'up', 'is', 'are'];

    // Check if all focus words (excluding stopwords) present in target string
    function allFocusWordsPresent(haystack, keyword) {
        let text = cleanText(haystack);
        let focusWords = cleanText(keyword).split(/\s+/).filter(w => w && !stopwords.includes(w));
        if (!focusWords.length) return false;
        return focusWords.every(word => text.includes(word));
    }
    // Keyword density: count phrase AND all-words (average for natural writing)
    function keywordDensity(text, keyword) {
        let plain = cleanText(text);
        let totalWords = plain.split(/\s+/).length;
        let phrase = cleanText(keyword).trim();
        let focusWords = phrase.split(/\s+/).filter(w => w && !stopwords.includes(w));
        if (!totalWords || !focusWords.length) return 0;
        // Count full phrase matches
        let phraseRegex = new RegExp(`\\b${phrase}\\b`, "g");
        let phraseCount = (plain.match(phraseRegex) || []).length;
        // Count all focus word matches (not strict order)
        let wordsCount = 0;
        focusWords.forEach(word => {
            let wordRegex = new RegExp(`\\b${word}\\b`, "g");
            wordsCount += (plain.match(wordRegex) || []).length;
        });
        // Average (gives fairer score for natural language)
        let avgCount = (phraseCount + wordsCount / focusWords.length) / 2;
        return (avgCount / totalWords) * 100;
    }
    ...
    function setDotColor(dotId, color) {
        ...
        dot.className = 'seo-dot ' + color;
    }

    function checkSEOandReadability() {
        ...
        if (window.tinymce && tinymce.get('tinymce-content')) {
            contentHTML = tinymce.get('tinymce-content').getContent() || '';
        } else {
            contentHTML = document.getElementById('tinymce-content').value || '';
        }
        // Parse HTML to extract text
        let doc = document.createElement("div");
        doc.innerHTML = contentHTML;
        let plain = doc.innerText || "";

        // --- Title Length Rule: Accept 40-70 as "Good" ---
        let titleStatus = document.getElementById('title-status');
        let titleLen = title.length;
        document.getElementById('title-length').innerText = `${titleLen}/70 chars`;
        if (titleLen >= 40 && titleLen <= 70) {
            titleStatus.innerText = "Title: Good";
            titleStatus.className = "badge bg-success";
            setDotColor('dot-title', 'green');
        } else if (titleLen > 0) {
            titleStatus.innerText = "Title: Ok";
            titleStatus.className = "badge bg-warning text-dark";
            setDotColor('dot-title', 'yellow');
        } else {
            titleStatus.innerText = "Title: Missing";
            titleStatus.className = "badge bg-danger";
            setDotColor('dot-title', 'red');
        }

        // Description Check
        let descStatus = document.getElementById('desc-status');
        let descLen = desc.length;
        document.getElementById('desc-length').innerText = `${descLen}/160 chars`;
        if (descLen >= 120 && descLen <= 160) {
            descStatus.innerText = "Description: Good";
            descStatus.className = "badge bg-success";
            setDotColor('dot-desc', 'green');
        } else if (descLen > 0) {
            descStatus.innerText = "Description: Ok";
            descStatus.className = "badge bg-warning text-dark";
            setDotColor('dot-desc', 'yellow');
        } else {
            descStatus.innerText = "Description: Missing";
            descStatus.className = "badge bg-danger";
            setDotColor('dot-desc', 'red');
        }

        // Focus Keyword Check (all focus words present in title/content)
        let keywordStatus = document.getElementById('keyword-status');
        let keywordCheck = document.getElementById('keyword-check');
        if (keyword.length > 0) {
            let inTitle = allFocusWordsPresent(title, keyword);
            let inContent = allFocusWordsPresent(plain, keyword);
            if (inTitle && inContent) {
                keywordStatus.innerText = "Focus Keyword: Good";
                keywordStatus.className = "badge bg-success";
                keywordCheck.innerText = "All focus words found in title & content";
                setDotColor('dot-keyword', 'green');
            } else if (inTitle || inContent) {
                keywordStatus.innerText = "Focus Keyword: Ok";
                keywordStatus.className = "badge bg-warning text-dark";
                keywordCheck.innerText = "All focus words found in " + (inTitle ? "title" : "content");
                setDotColor('dot-keyword', 'yellow');
            } else {
                keywordStatus.innerText = "Focus Keyword: Not found";
                keywordStatus.className = "badge bg-danger";
                keywordCheck.innerText = "Not all focus words found in title or content";
                setDotColor('dot-keyword', 'red');
            }
        } else {
            keywordStatus.innerText = "Focus Keyword: Missing";
            keywordStatus.className = "badge bg-secondary";
            keywordCheck.innerText = "";
            setDotColor('dot-keyword', 'gray');
        }

        // Readability Analysis
        const score = fleschReadingEase(plain);
        let readabilityScore = document.getElementById('readability-score');
        let detail = document.getElementById('readability-detail');
        if (score >= 60) {
            readabilityScore.innerText = "Readability: Good";
            readabilityScore.className = "badge bg-success";
            setDotColor('dot-readability', 'green');
            detail.innerText = `Flesch score: ${score.toFixed(0)} (easy to read)`;
        } else if (score >= 30) {
            readabilityScore.innerText = "Readability: Ok";
            readabilityScore.className = "badge bg-warning text-dark";
            setDotColor('dot-readability', 'yellow');
            detail.innerText = `Flesch score: ${score.toFixed(0)} (medium)`;
        } else {
            readabilityScore.innerText = "Readability: Hard";
            readabilityScore.className = "badge bg-danger";
            setDotColor('dot-readability', 'red');
            detail.innerText = `Flesch score: ${score.toFixed(0)} (difficult)`;
        }

        // Extra SEO Checks
        let extraChecks = [];
        if (keyword) {
            // Focus keyword at start of title (ignore stopwords)
            let titleStart = cleanText(title).split(/\s+/).filter(w => w && !stopwords.includes(w))[0] || '';
            let focusFirst = cleanText(keyword).split(/\s+/).filter(w => w && !stopwords.includes(w))[0] || '';
            if (titleStart === focusFirst) {
                extraChecks.push('<span style="color:green;">✔</span> Focus keyword starts title');
            } else {
                extraChecks.push('<span style="color:red;">✖</span> Focus keyword does not start title');
            }
            // Keyword in first paragraph (all focus words, flexible)
            let p = doc.querySelector('p');
            let firstPara = '';
            if (p) firstPara = p.innerText;
            if (firstPara && allFocusWordsPresent(firstPara, keyword)) {
                extraChecks.push('<span style="color:green;">✔</span> All focus words in first paragraph');
            } else {
                extraChecks.push('<span style="color:red;">✖</span> Not all focus words in first paragraph');
            }
            // Keyword density
            let density = keywordDensity(plain, keyword);
            if (density >= 0.5 && density <= 3) {
                extraChecks.push(
                    `<span style="color:green;">✔</span> Focus keyword density is good (${density.toFixed(2)}%)`
                );
            } else {
                extraChecks.push(
                    `<span style="color:red;">✖</span> Focus keyword density is not optimal (${density.toFixed(2)}%)`
                );
            }
        }
        // Outbound/internal links (in HTML)
        if (/https?:\/\//.test(contentHTML)) {
            extraChecks.push('<span style="color:green;">✔</span> At least one link present in content');
        } else {
            extraChecks.push('<span style="color:red;">✖</span> No links in content');
        }
        // Images with alt attribute (parse HTML)
        let imgs = doc.querySelectorAll('img[alt]');
        if (imgs.length > 0) {
            extraChecks.push('<span style="color:green;">✔</span> At least one image with alt attribute');
        } else {
            extraChecks.push('<span style="color:red;">✖</span> No image with alt attribute');
        }
        document.getElementById('extra-checks-list').innerHTML = extraChecks.map(e => `<li>${e}</li>`).join('');
...
let isSlugManuallyEdited = false;
    ...
    // If user types in slug field, mark as manually edited and clean
    if (slugInput) {
    ...
    // Auto-fill slug from title unless user already edited slug
    ...
    // Optional: If user clears slug, re-enable auto mode
    if (slugInput) {
...
function updateSocialPreview() {
        const title = document.getElementById('meta_title').value || 'Blog Title';
        const desc = document.getElementById('meta_description').value || 'Meta description preview here.';
        const slug = slugInput.value || 'your-slug-here';
        const url = window.location.origin + '/blog/' + slug;

        let metaImage = document.getElementById('meta_image').value;
        let fileInput = document.getElementById('blog-image');
        let previewImage = document.getElementById('preview-image');

        if (metaImage && metaImage.trim().length > 0) {
            previewImage.src = metaImage;
        ...
        document.getElementById('preview-description').innerText = desc;
       ...
       tinymce.init({
        ...
        ['meta_title', 'meta_description', 'focus_keyword', 'meta_image'].forEach(function(id) {
            let el = document.getElementById(id);
            if (el) el.addEventListener('input', checkSEOandReadability);
        });
        checkSEOandReadability();
    ...
    ['meta_title', 'meta_description', 'meta_image', 'url_slug'].forEach(function(id){
        let el = document.getElementById(id);
        if(el) el.addEventListener('input', updateSocialPreview);
    });

    // Also update when feature image file is selected
    let fileInput = document.getElementById('blog-image');
    if (fileInput) {
        fileInput.addEventListener('change', updateSocialPreview);
    }
